...
class News_data():
    ...
    def select_by_paramters(self,parameter):
        ...
        if keywords == None or keywords== "":
            ...
            data = [item for item in news_data.find({}, {"_id": 0}).sort([{"collection_time",-1}]).limit(limit).skip(page * limit)]

        else:
            count = news_data.find({'source': {'$regex':keywords}}).count()
            data = [item for item in
                    news_data.find({'source': {'$regex':keywords}}, {"_id": 0}).sort([{"collection_time",-1}]).limit(limit).skip(
                           page * limit)]
...
if __name__ == '__main__':
    ...
    con = News_data()
    # con.insert(data)
    paramters = {"page":1,"limit":10,"keyword":""}
