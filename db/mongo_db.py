...
class News_data():
    ...
    def select_by_paramters(self,parameter):
        ...
        if keywords == None or keywords== "":
            ...
            data = [item for item in news_data.find({}, {"_id": 0}).limit(limit).skip(page * limit)]

        else:
            count = news_data.find({'title': {"$in": [keywords]}}).count()
            data = [item for item in
                    news_data.find({'title': {"$in": [keywords]}}, {"_id": 0}).limit(limit).skip(
                           page * limit)]
