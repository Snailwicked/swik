...
'''
https://bigbrotherwatch.org.uk/media-category/press-releases/page/{}/
https://bigbrotherwatch.org.uk/media-category/press-coverage/page/{}/
https://bigbrotherwatch.org.uk/media-category/opinion-pieces/
'''


for i in range(1,3):
    url = "https://bigbrotherwatch.org.uk/media-category/opinion-pieces/page/{}/".format(i)
    html = requests.get(url).text
    html_tree = etree.HTML(html)
    herfs = html_tree.xpath("//h2[@class= 'entry-title']//a//@href")
    for item in herfs:
        data = {}
        try:
        ...
        data["title"] = str(html_tree.xpath("//h1[@class= 'entry-title']//text()")[0]).replace("'","").replace("\"","")
        data["translat_title"] = translation(data["title"])
        timeArray = time.localtime(int(time.mktime(date_parser(html_tree.xpath("//time[contains(@class , 'entry-date published')]//@datetime")[0]).timetuple())))
        otherStyleTime = time.strftime("%Y-%m-%d %H:%M:%S", timeArray)
        data["create_time"] = otherStyleTime
        data["content"] = str(''.join([item + "\n" for item in html_tree.xpath("//div[@class= 'entry-content inside']//p//text()")]).replace("'","").replace("\"",""))
        try:
    ...
    #
        data["country"] = "英国"
        data["original_link"] = item
        data["longitude"] = 138.250000
        data["web_site"] = "Big Brother Watch"
