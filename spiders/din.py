...
'''
https://www.dni.gov/index.php/newsroom/news-articles?start={}
https://www.dni.gov/index.php/newsroom/reports-publications?start=5
https://www.dni.gov/index.php/newsroom/press-releases?start=30
https://www.dni.gov/index.php/newsroom/congressional-testimonies
https://www.dni.gov/index.php/ncsc-newsroom?start=5
https://www.dni.gov/index.php/nctc-newsroom?start=5
'''

# timeArray = time.localtime(int(time.mktime(date_parser("Friday, 06 September 2019 12:15").timetuple())))
# otherStyleTime = time.strftime("%Y-%m-%d %H:%M:%S", timeArray)
# print(otherStyleTime)
for i in range(1):

    url = "https://www.dni.gov/index.php/nctc-newsroom?start={}".format(i*5)
    print(url)
    html = requests.get(url).text
    html_tree = etree.HTML(html)
    herfs = html_tree.xpath("//h3[@class= 'catItemTitle']//a//@href")
    titles = html_tree.xpath("//h3[@class= 'catItemTitle']//a//text()")
    times = html_tree.xpath("//span[@class= 'catItemDateCreated']//text()")

    for i in range(len(herfs)):
        data = {}
        html = requests.get("https://www.dni.gov"+herfs[i]).text
        html_tree = etree.HTML(html)
        data["title"] = str(titles[i]).replace("'","").replace("\"","").replace("\\n","").replace("\\t","").strip()
    ...
    # #
    #
        data["content"] = str(''.join([item + "\n" for item in html_tree.xpath("//div[@class= 'itemFullText']//text()")]).replace("'","").replace("\"",""))
        try:
    ...
    #
        data["country"] = "美国"
        data["original_link"] = "https://www.dni.gov"+herfs[i]
        data["longitude"] = 138.250000
        data["web_site"] = "国家情报总监办公室"
