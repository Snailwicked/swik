...
'''
https://intelligence.house.gov/news/documentquery.aspx?Year=2019&Page=2
'''
for i in range(1,8):

    url = "https://intelligence.house.gov/news/documentquery.aspx?Year=2019&Page={}".format(i)
    print(url)
    html = requests.get(url).text
    html_tree = etree.HTML(html)
    herfs = html_tree.xpath("//div[@class = 'news-texthold']//h3//a//@href")
    titles =  html_tree.xpath("//div[@class = 'news-texthold']//h3//a//text()")
    times =  html_tree.xpath("//div[@class= 'news-texthold']//div[@class= 'newsie-details']//time//text()")
    ...
    for i in range(len(herfs)):
        data = {}
        html = requests.get("https://intelligence.house.gov/news/"+herfs[i]).text
        html_tree = etree.HTML(html)
    ...
    # # # #
    # # #
        data["content"] = str(''.join([item + "\n" for item in html_tree.xpath("//div[@class= 'newsbody clearfix']//p//text()")]).replace("'","").replace("\"",""))
        try:
    ...
    # #
        data["country"] = "美国"
        data["original_link"] = "https://intelligence.house.gov/news/"+herfs[i]
        data["longitude"] = 138.250000
        data["web_site"] = "http://intelligence.house.gov"
