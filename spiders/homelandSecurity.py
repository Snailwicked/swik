...
'''
http://www.homelandsecuritynewswire.com/topics/government-policy?page=1
http://www.homelandsecuritynewswire.com/topics/emergency-police-mil?page=5
'''


for i in range(1,6):

    url = "http://www.homelandsecuritynewswire.com/topics/emergency-police-mil?page={}".format(i)
    print(url)
    html = requests.get(url).text
    html_tree = etree.HTML(html)
    herfs = html_tree.xpath("//ul[@id= 'main-stories-list']//li[contains(@class,'views-row-')]//div//h3//a//@href")
    titles = html_tree.xpath("//ul[@id= 'main-stories-list']//li[contains(@class,'views-row-')]//div//h3//a//text()")
    # times = html_tree.xpath("//div[@id= 'main']//div[contains(@class,'views-row views')]//span[@class='date']//text()")
    # print(len(herfs)==len(titles))
    # print(herfs)
    # print(titles)
    # print(times)
    for i in range(len(herfs)):
        data = {}
        html = requests.get("http://www.homelandsecuritynewswire.com"+herfs[i]).text
        html_tree = etree.HTML(html)
    ...
    # #
    #
        data["content"] = str(''.join([item + "\n" for item in html_tree.xpath("//div[@id= 'main-content']//p//text()")]).replace("'","").replace("\"",""))
        try:
            timeArray = time.localtime(int(
                time.mktime(date_parser(html_tree.xpath("//meta[@name= 'dcterms.date']//@content")[0]).timetuple())))
    ...
    #
        data["country"] = "美国"
        data["original_link"] = "http://www.homelandsecuritynewswire.com"+herfs[i]
        data["longitude"] = 138.250000
        data["web_site"] = "国土安全新闻"
