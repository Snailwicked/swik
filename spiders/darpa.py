...
for i in range(4):
    url = "https://www.darpa.mil/news?PP={}".format(i)
    html = requests.get(url).text
    html_tree = etree.HTML(html)
    herfs = html_tree.xpath("//h2[@class= 'listing__link']//a//@href")
    news_herfs = ["https://www.darpa.mil"+item for item in herfs]
    for sub_url in news_herfs:
        data = {}
        html = requests.get(sub_url).text
        html_tree = etree.HTML(html)
        data["title"] = str(html_tree.xpath("//h1[@class= 'detail__title']//text()")[0]).replace("'","").replace("\"","")
        data["translat_title"] = translation(data["title"])
        timeArray = time.localtime(int(time.mktime(date_parser(html_tree.xpath("//div[@class= 'detail__date']//text()")[0]).timetuple())))
        otherStyleTime = time.strftime("%Y-%m-%d %H:%M:%S", timeArray)
        data["create_time"] = otherStyleTime



        data["content"] = str(''.join([item + "\n" for item in html_tree.xpath("//div[@class= 'detail__content']//p//text()")]).replace("'","").replace("\"",""))
        try:
