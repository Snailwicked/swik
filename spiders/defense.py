...
'''
https://www.defense.gov/Newsroom/Transcripts/?Page=0
https://www.defense.gov/Newsroom/Advisories/?Page=0
https://www.defense.gov/Newsroom/Releases/?Page=0
https://www.defense.gov/Newsroom/Publications/?Page=0
https://www.defense.gov/Newsroom/Advisories/?Page=0
https://www.defense.gov/Newsroom/Speeches/?Page=0
...
for i in range(1,2):
    ...
    url = "https://www.defense.gov/Newsroom/Speeches/?Page={}".format(i)
    html = requests.get(url).text
    html_tree = etree.HTML(html)
    herfs = html_tree.xpath("//div[@class= 'lside']//a//@href")
    titles = html_tree.xpath("//div[@class= 'lside']//a//text()")
    times = html_tree.xpath("//div[@class= 'lside']//div[@class= 'timecontainer']//time//text()")

    for i in range(len(herfs)):
        data = {}
        try:
            html = requests.get(herfs[i]).text
        except:
            continue
        html_tree = etree.HTML(html)
        ...
        data["content"] = str(''.join([item + "\n" for item in html_tree.xpath("//div[@class= 'body']//text()")]).replace("'","").replace("\"",""))
        try:
