...
class Crawler:
    ...
    def xpath_urls(self, urls,limit):
        if limit == self.limit:
            ...
            filter_rule =self.rule["filter_rule"].replace(" ","+").replace("\\\\","\\")
            if filter_rule:
                target_url = []
                catalogue = []
                for url  in urls:
                    for item in self.get_hrefs(url):
                        print(item)
                        regex_start = re.compile(filter_rule)
                        result = regex_start.findall(item)
...
if __name__ == '__main__':
    ...
    url = 'https://news.sina.com.cn/w/2020-01-06/doc-iihnzahk2306137.shtml'
    import re

    regex_start = re.compile("https://news.sina.com.cn/\w+/\d{4}-\d{2}-\d{2}/")

    result = regex_start.findall(url)
    print(result)