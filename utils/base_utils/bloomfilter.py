...
class filter:
    def __init__(self,path=None):
        ...
        if self.ispath:
            self.bf = ScalableBloomFilter.fromfile(open(self.path, 'rb'))
        else:
            self.bf = ScalableBloomFilter(initial_capacity=1000000, error_rate=0.00001,
    ...
    def tofile(self):
        self.bf.tofile(open(self.path, 'wb'))
